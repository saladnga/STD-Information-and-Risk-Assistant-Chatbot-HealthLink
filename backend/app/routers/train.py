...
from auth_utils import verify_token, ALLOW_UNAUTHENTICATED_TRAIN
...
async def verify_auth_optional(credentials: Optional[HTTPAuthorizationCredentials] = Depends(security)):
    """
    ...
    if ALLOW_UNAUTHENTICATED_TRAIN:
        logger.warning("Training endpoint is accessible without authentication (development mode)")
    ...
    if user_role not in ["admin", "trainer"]:
        logger.warning(f"User {payload.get('sub')} with role {user_role} attempted training")
...
async def training_status(user: Dict = Depends(verify_auth_optional)):
    ...
    return {
        "status": "available" if all(f["exists"] for f in status_info.values()) else "incomplete",
        "files": status_info,
        "metrics": metrics
    }
